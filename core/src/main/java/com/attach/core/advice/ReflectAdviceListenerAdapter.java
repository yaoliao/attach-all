...
//    }
     ...
     * 超过次数上限，则不再输出，命令终止
     *
     * @param session the process to be aborted
     * @param limit   the limit to be printed
     */
    protected void abortProcess(Session session, int limit, int requestId) {
        session.writeStr(requestId, "Command execution times exceed limit: " + limit + ", so command will exit. You can set it with -n option.\n");
        session.end();
    }
